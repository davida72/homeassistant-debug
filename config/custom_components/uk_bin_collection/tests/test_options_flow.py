...
async def test_options_flow_user_to_council_info(options_flow):
    ...
    # Mock the build_user_schema function to return a schema that includes our fields
    with patch("uk_bin_collection.utils.build_user_schema") as mock_build:
    ...
    assert options_flow.data[council_field] is not None, f"Council field '{council_field}' is None"
...
async def test_options_flow_council_info_to_advanced(options_flow):
    ...
    # We need to mock both build_council_schema and async_step_advanced
    with patch("uk_bin_collection.utils.build_council_schema") as mock_council_schema, \
         patch.object(options_flow, "async_step_advanced") as mock_advanced:
...
async def test_options_flow_council_info_to_selenium(options_flow):
    ...
    # Mock the build_user_schema function
    with patch("uk_bin_collection.utils.build_user_schema") as mock_build:
        mock_build.return_value = vol.Schema({
            vol.Required("name"): str,
            vol.Required("selected_council"): str,
    ...
    # Mock the build_council_schema function
    with patch("uk_bin_collection.utils.build_council_schema") as mock_build_council:
...
async def test_options_flow_selenium_to_advanced(options_flow):
    ...
    # Mock the selenium schema and validation
    with patch("uk_bin_collection.utils.build_selenium_schema") as mock_selenium_schema, \
         patch("uk_bin_collection.utils.validate_selenium_config") as mock_validate:
...
async def test_options_flow_advanced_to_create_entry(options_flow):
    ...
    with patch("uk_bin_collection.utils.build_advanced_schema") as mock_build_advanced, \
         patch("uk_bin_collection.utils.prepare_config_data") as mock_prepare, \
         patch("uk_bin_collection.options_flow.get_advanced_defaults") as mock_defaults:
