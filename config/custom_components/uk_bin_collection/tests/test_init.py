...
async def test_async_setup_entry(hass, config_entry):
    ...
    ukbcd_mock.run = lambda: json.dumps({"bins": [
        {"type": "Recycling", "collectionDate": datetime.now().strftime("%d/%m/%Y")}
    ]})
    ...
    with patch("custom_components.uk_bin_collection.UKBinCollectionApp", return_value=ukbcd_mock):
...
async def test_household_bin_coordinator_update(hass):
    ...
    bin_data = {
        "bins": [
            {"type": "Recycling", "collectionDate": (dt_util.now() + timedelta(days=2)).strftime("%d/%m/%Y")},
            {"type": "General Waste", "collectionDate": (dt_util.now() + timedelta(days=5)).strftime("%d/%m/%Y")},
            {"type": "Garden Waste", "collectionDate": (dt_util.now() - timedelta(days=2)).strftime("%d/%m/%Y")},
        ]
    }
